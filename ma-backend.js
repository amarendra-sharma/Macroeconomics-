/* ============================================================================
   ma-backend.js  —  shared client module for the Applied Macroeconomics arena suite.

   One file, imported by all 16 arenas, that provides:
     * Supabase auth (email + password, separate Applied Macroeconomics accounts)
     * an access gate (ma_has_access) that unlocks arenas after the $25 purchase
     * vaulted grading: submitArena() posts to the ma-grade Edge Function and
   ...
   Usage in an arena (see the retrofit example in the delivery notes):
     ...
     <script src="ma-backend.js"></script>
     ... then call MABackend.gradeCase(arenaSlug, caseIndex, chosenCode, cb)
...
(function (global) {
  ...
  var GRADE_FN_URL = SUPABASE_URL + "/functions/v1/ma-grade";
  var GRADE_QUIZ_FN_URL = SUPABASE_URL + "/functions/v1/ma-grade-quiz";
  ...
  var accessCache = null; /* cached ma_has_access result */
  ...
  function signUp(email, password, displayName, cb) {
    ...
    sb.auth.signUp({ email: email, password: password }).then(function (res) {
      ...
      /* create the ma_person row (RLS lets a user insert their own) */
      var uid = res.data && res.data.user ? res.data.user.id : null;
      if (uid) {
        sb.from("ma_person").upsert(
  ...
  function signIn(email, password, cb) {
    ...
    sb.auth.signInWithPassword({ email: email, password: password }).then(function (res) {
      ...
      sb.rpc("ma_reconcile_activation").then(function () {
        cb({ ok: true });
  ...
  function checkAccess(cb) {
    ...
    sb.rpc("ma_has_access").then(function (res) {
  ...
  function gradeCase(arenaSlug, caseIndex, submitted, cb, attemptNo, review) {
    ...
    if (review && typeof review === "object") {
      ...
      /* The macro arenas draw their cases from the shared question bank, so the
         server needs the generator id and seed to REGENERATE the case and grade
         it. The micro version of this function forwarded only prompt_text and
         labels, which is enough for a hardcoded arena key but leaves a
         bank-driven arena with nothing for the grader to compare against. */
      if (review.generator_id) { payload.generator_id = review.generator_id; }
      if (typeof review.seed === "number") { payload.seed = review.seed; }
      if (review.course_id) { payload.course_id = review.course_id; }
    }
  ...
  /* ---- GENERIC EDGE-FUNCTION CALL ----------------------------------------
     ...
     no session / network error. Used by the attendance gate (ma-attendance). */
  ...
  /* ---- QUIZ GRADING (randomized items, server regenerates from seed) ------
     ...
     an ma_exam_attempt row per item. cb receives
  ...
  function getMyScores(cb) {
    ...
    sb.from("ma_score").select("arena_slug,attempted,correct,best_streak,score_pct")
      .then(function (res) { cb(res && res.data ? res.data : []); })
      .catch(function () { cb([]); });
  }

  /* ---- CONTENT GATE (freemium) -------------------------------------------
     Call MABackend.gateContent() from a gated chapter/arena. It checks access
  ...
  function buildOverlay(mode) {
    var ov = global.document.getElementById("ma-gate-overlay");
    if (ov) { ov.parentNode.removeChild(ov); }
    ov = global.document.createElement("div");
    ov.id = "ma-gate-overlay";
    ...
    var signInBlock =
      "<div style='margin-top:1.25rem;text-align:left;'>" +
        "<input id='ma-gate-email' type='email' placeholder='email' style='width:100%;padding:0.6rem 0.75rem;margin-bottom:0.5rem;border:1px solid #cbd5e1;border-radius:8px;font-size:0.9rem;'>" +
        "<input id='ma-gate-pass' type='password' placeholder='password' style='width:100%;padding:0.6rem 0.75rem;margin-bottom:0.75rem;border:1px solid #cbd5e1;border-radius:8px;font-size:0.9rem;'>" +
        "<button id='ma-gate-signin' style='width:100%;padding:0.65rem;border:none;border-radius:8px;background:#0f3d9e;color:#fff;font-weight:600;font-size:0.9rem;cursor:pointer;'>Sign in</button>" +
        "<div id='ma-gate-msg' style='color:#991b1b;font-size:0.8rem;margin-top:0.5rem;min-height:1em;'></div>" +
      "</div>";
    var unlockBlock =
      "<button id='ma-gate-unlock' style='margin-top:1.25rem;width:100%;padding:0.7rem;border:none;border-radius:8px;background:#b87408;color:#fff;font-weight:700;font-size:0.95rem;cursor:pointer;'>Unlock the full course</button>" +
      "<button id='ma-gate-signout' style='margin-top:0.5rem;width:100%;padding:0.55rem;border:1px solid #cbd5e1;border-radius:8px;background:transparent;color:#334155;font-weight:600;font-size:0.85rem;cursor:pointer;'>Sign out</button>";
    var body = mode === "unlock"
      ? "<p style='color:#475569;font-size:0.95rem;line-height:1.6;margin-top:0.5rem;'>You're signed in, but this chapter is part of the full course. Chapters 1&ndash;2 and the Market Sandbox are free; unlocking gives you all 18 chapters, all 16 arenas, exams, and your gradebook.</p>" + unlockBlock
      : "<p style='color:#475569;font-size:0.95rem;line-height:1.6;margin-top:0.5rem;'>This chapter is part of the full course. Sign in to your Applied Macroeconomics account to continue, or head back to the free preview.</p>" + signInBlock;
    ov.innerHTML =
    ...
    var si = global.document.getElementById("ma-gate-signin");
    if (si) {
      si.addEventListener("click", function () {
        var e = global.document.getElementById("ma-gate-email");
        var p = global.document.getElementById("ma-gate-pass");
        var m = global.document.getElementById("ma-gate-msg");
    ...
    var un = global.document.getElementById("ma-gate-unlock");
    if (un) {
    ...
    var so = global.document.getElementById("ma-gate-signout");
  ...
  /* ---- PRICING (public, from ma_public_pricing view) ---------------------
  ...
  function getPricing(cb) {
    ...
    if (!init()) { cb(null); return; }
    sb.from("ma_public_pricing").select("price_cents,currency,stripe_link").maybeSingle()
      .then(function (res) {
        if (res && res.data) {
          pricingCache = {
