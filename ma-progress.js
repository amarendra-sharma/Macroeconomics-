/* ======================================================================
   ma-progress.js  —  Applied Macroeconomics shared progress reporter
   ----------------------------------------------------------------------
   ...
   USAGE in a host file:
     ...
     ... then call, e.g.:
     MAProgress.recordQuiz(6, 80, true);
     MAProgress.markReadingDone(6);
     MAProgress.recordArena('cooperative', 0.82, 'win', { rounds: 10 });
...
(function (global) {
  ...
  if (!global.supabase || !global.supabase.createClient) {
    global.MAProgress = makeNoop('supabase-js not loaded');
    global.MAProgressCompat = global.MAProgress;
    return;
  }

  var sb = global.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
  try { console.log('%cim-progress build: macro-2026-09 | project: ' + SUPABASE_URL, 'color:#0f3d9e;font-weight:bold'); } catch (e) {}
  ...
  function resolveContext() {
    ...
    ready = sb.auth.getSession().then(function (res) {
      ...
      return sb.from('ma_enrollment').select('course_id,status')
        ...
        .then(function (enr) {
          ...
          return sb.from('ma_course').select('id,owner_id').in('id', cids)
            .then(function (cr) {
  ...
  function markReadingDone(chapter) {
    return resolveContext().then(function (c) {
      if (!c || !c.courseId) {
        try { console.warn('ma-progress: reading NOT saved for ch ' + chapter + ' -- no logged-in student or no resolved course.'); } catch (e) {}
        return null;
      }
      return sb.from('ma_chapter_progress').upsert({
        student_id: c.user.id,
        course_id: c.courseId,
        chapter: chapter,
        reading_done: true
      }, { onConflict: 'student_id,course_id,chapter' }).then(function (r) {
        if (r && r.error) { try { console.warn('ma-progress: reading save FAILED for ch ' + chapter + ': ' + r.error.message); } catch (e) {} }
        else { try { console.log('ma-progress: reading saved for ch ' + chapter + ' (course ' + c.courseId + ')'); } catch (e) {} }
        return r;
      });
    }).catch(function (e) {
      try { console.warn('ma-progress: reading save threw for ch ' + chapter + ': ' + (e && e.message ? e.message : e)); } catch (er) {}
      return null;
    });
  }
  ...
  // Record an arena session. In Applied Macroeconomics, arenas persist their own per-case
  // results (ma_attempt) through their game code, so this reporter does NOT write
  ...
  // An arena calls MAProgress.initArena({ slug, getSession }) once at load.
  ...
  function initArena(opts) {
    ...
    // explicitly signals completion via MAProgress.reportArenaNow(). We do NOT
    ...
    // single write (recommended: call MAProgress.reportArenaNow() at match end).
    global.MAProgress.reportArenaNow = flush;
  }
  ...
  /* ====================================================================
     ...
     what remains. Caller invokes MAProgress.trackReading(chapterEl, n, mins).
     ==================================================================== */
  ...
  function trackReading(chapterEl, chapterNum, estMinutes) {
    ...
    // sections (verified uniform across all 18 chapters), with <h3> subsections in
  ...
  // Back-compat: the textbook and arenas were written to call MAProgressCompat.* and
