/* ==========================================================================
   ma-exams.js — Exam assembly engine + instructor exam builder + student runner
   ...
   Depends on: MAGenerators (generate/grade/list), MADiagrams (via MAQuiz), MAQuiz.
   Exposes: window.MAExams with:
     ...
     - renderExam(examSpec, mountEl, cfg) -> student-facing exam runner (uses MAQuiz.quiz)
   ========================================================================== */
(function () {
  "use strict";
  ...
  var MIDTERM_CHAPTERS = [1, 2, 3, 4, 5, 6, 7, 8, 9];
  var FINAL_CHAPTERS = [10, 11, 12, 13, 14, 15, 16, 17, 18];
  ...
  function chapterPool(chapter, opts) {
    opts = opts || {};
    var all = window.MAGenerators.list();
    var out = [];
  ...
  function itemPoints(id, seed) {
    var q = window.MAGenerators.generate(id, seed || 1);
  ...
  /* ======================================================================
     STUDENT EXAM RUNNER — wraps MAQuiz.quiz with an assembled item list.
     examSpec = { title, items:[{generatorId, seed, points}], timeLimitMinutes,
                  shuffle:bool, proctored:bool }
     cfg = { onSubmit(results), gradeEndpoint, ... } forwarded to MAQuiz.
     ====================================================================== */
  function renderExam(examSpec, mountEl, cfg) {
    ...
    if (window.MAQuiz && window.MAQuiz.quiz) {
      return window.MAQuiz.quiz(spec, mountEl, cfg);
    }
    mountEl.innerHTML = "<p style='color:#991b1b'>Exam engine (MAQuiz) not loaded.</p>";
    return null;
  }
  ...
  function renderBuilder(mountEl, cfg) {
    cfg = cfg || {};
    var allowChapters = cfg.allowChapters || rangeArr(1, 18);
    var state = {
      preset: "custom",
    ...
    root.className = "ma-exam-builder";
    mountEl.innerHTML = "";
    ...
    function presetHtml() {
      return "<div class='xb-card'><div class='xb-row'>" +
        presetBtn("midterm", "Midterm (40, Ch 1&ndash;9)") +
        presetBtn("final", "Final (50, Ch 10&ndash;18)") +
    ...
    function chaptersHtml() {
      ...
      return "<div class='xb-card'><div class='xb-cardhead'><h3>Chapters</h3>" +
        ...
        "<button class='xb-mini' id='xb-selfirst'>First half (1&ndash;9)</button>" +
        "<button class='xb-mini' id='xb-selsecond'>Second half (10&ndash;18)</button></div>" +
  ...
  function injectStyles() {
    if (document.getElementById("ma-exam-builder-styles")) { return; }
    var css =
      ".ma-exam-builder{font-family:Inter,system-ui,sans-serif;color:#0f172a;max-width:860px;margin:0 auto}" +
      ".ma-exam-builder h2{font-size:1.5rem;margin:0 0 .25rem}.ma-exam-builder h3{font-size:1.05rem;margin:0 0 .6rem}" +
